package com.dirtfy.ppp.test.data.source.impl.firestore.account
...
class AccountFireStore @Inject constructor(
    ...
    private fun readAllAccount(
        accountRef: QuerySnapshot,
        recordRef: QuerySnapshot
    ): List<DataAccount> {
        return accountRef.documents.map {
            it.toObject(FireStoreAccount::class.java)!!
        }.map { account ->
            val balance = recordRef.documents
                .map {
    ...
    override suspend fun readAllAccount(): List<DataAccount> {
        val accountSnapshot = accountRef.get().await()
        val recordSnapshot = recordRef
            .whereNotIn(
                "type",
                DataRecordType.entries.map { it.name }
            ).get().await()

        return readAllAccount(accountSnapshot, recordSnapshot)
    }
    ...
    private fun readAllRecord(
        ...
        val documents = recordSnapshot.documents
        var result = 0
    ...
    override fun accountStream(): Flow<List<DataAccount>>
    = callbackFlow {
    ...
    override fun accountRecordStream(accountNumber: Int): Flow<List<DataAccountRecord>>
    = callbackFlow {
        val targetRecordRef = recordRef
                .whereEqualTo("type", "$accountNumber")
                .orderBy("timestamp")
        ...
        val recordSubscription = targetRecordRef.addSnapshotListener { snapshot, error ->
            ...
            // Sends events to the flow! Consumers will get the new events
            try {
                val recordList = readAllRecord(snapshot)
