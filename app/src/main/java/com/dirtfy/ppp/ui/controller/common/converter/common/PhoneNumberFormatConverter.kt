...
object PhoneNumberFormatConverter {
    ...
    private fun formatNumber(cleaned: String, prefix: String, startIndex: Int, middleIndex: Int, endIndex: Int): String {
        return when {
    ...
    fun formatPhoneNumber(input: String): String {
        ...
        return when {
            cleaned.startsWith("02") -> formatNumber(cleaned, "02", 2, 5, 9)
            cleaned.startsWith("010") -> formatNumber(cleaned, "010", 3, 7, 11)
            areaCodes.any { cleaned.startsWith(it) } -> formatNumber(cleaned, cleaned.substring(0, 3), 3, 6, 10)
            else -> cleaned
        }
    }
}
